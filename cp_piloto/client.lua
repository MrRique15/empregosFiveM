...
emP = Tunnel.getInterface("cp_piloto")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local blips = nil
...
local CoordenadaY = -2957.57       -----   -930.73, -2957.57, 13.94
				...
				if distance <= 30.0 then
					...
					if distance <= 1.2 then
						...
						if IsControlJustPressed(1,38) and emP.checkPermission() then
							...
							TriggerEvent("Notify","bom","<b>Elysium:</b> Olá, <b>Piloto(a)</b>!",4000)
							TriggerEvent("Notify","bom","<b>Elysium:</b> Um passageiro já está a sua espera!",5000)
							TriggerServerEvent("colocar-uniforme:piloto")
						end
					end
				end
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if emservico then
			...
			if IsControlJustPressed(1,168) and (IsVehicleModel(vehicle,GetHashKey("supervolito")) or IsVehicleModel(vehicle,GetHashKey("supervolito2")) or IsVehicleModel(vehicle,GetHashKey("buzzard2")) or IsVehicleModel(vehicle,GetHashKey("frogger")) or IsVehicleModel(vehicle,GetHashKey("maverick"))) then
				RemoveBlip(blips)
				if DoesEntityExist(passageiro) then
					TaskLeaveVehicle(passageiro,vehicle,262144)
					TaskWanderStandard(passageiro,10.0,10)
				...
				checkped = true
				emservico = false
				TriggerServerEvent("tirar-uniforme:piloto")
