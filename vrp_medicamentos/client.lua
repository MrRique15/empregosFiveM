...
job = Tunnel.getInterface("vrp_medicamentos")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- INICIANDO TRABALHO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('vrp_medicamentos:permissao')
...
Citizen.CreateThread(function()
	while true do
		...
		if not emservico then
			if GetDistanceBetweenCoords(GetEntityCoords(ped), 95.1, -223.57, 54.64,true) <= 30 then
				timeDistance = 5
			    DrawText3D(95.1, -223.57, 54.64+0.47, "~w~Entregador", 1.0, 4)
			    DrawText3D(95.1, -223.57, 54.64+0.33, "~b~Remédios", 0.9, 1)
			    DrawMarker(39, 95.1, -223.57, 54.64-0.1, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 65, 105, 255, 50, 0, true, false, false)
			    if GetDistanceBetweenCoords(GetEntityCoords(ped), 95.1, -223.57, 54.64,true) <= 1 then
        	        if IsControlJustPressed(0,38) then	
					    TriggerServerEvent('vrp_medicamentos:permissao')
					end
				end
			end
		elseif emservico and parte == 0 then
			timeDistance = 5
			if GetDistanceBetweenCoords(GetEntityCoords(ped), 95.14, -211.89, 54.5,true) <= 20 then
		        DrawMarker(21,95.14, -211.89, 54.5-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
		        if GetDistanceBetweenCoords(GetEntityCoords(ped), 95.14, -211.89, 54.5,true) <= 1 then
                    if IsControlJustPressed(0,38) then	
                    	Fade(1200)
				        TriggerEvent("Notify","importante","Voce pegou a <b>Van</b> na garagem, pegue os <b>Medicamentos</b> e coloque nela!")
		                spawnVan()
                        parte = 1
				    end
			    end
		...
		elseif emservico and parte == 1 then
			...
			local distance = GetDistanceBetweenCoords(GetEntityCoords(ped),97.93, -228.52, 54.67,true)
			...
			if distance <= 50 and not pegando and portaaberta then
				timeDistance = 5
				DrawMarker(21,97.93, -228.52, 54.67-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
				if distance < 1.5 and not IsPedInAnyVehicle(ped) then
                    if IsControlJustPressed(0,38) and not andamento then
                    	if quantidade < 10 then
                            ...
                            vRP._CarregarObjeto("anim@heists@box_carry@","idle","apa_mp_h_acc_box_trinket_01",50,28422)
                        else
			...
			if pegando and distance2 <= 1.5 and not IsPedInAnyVehicle(ped) then
				timeDistance = 5
                if model == 444171386 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    ...
                    if IsControlJustPressed(0,38) and andamento then
                        ...
                        if quantidade == 10 then
                        	TriggerEvent("Notify","importante","Voce colocou <b>"..quantidade.."/10 Medicamentos</b> na <b>Van</b>, feche as portas traseiras, entre nela e aguarde até receber uma <b>Entrega</b>!")
                        	vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                        	portaaberta = false
                        	time = math.random(10000,15000)
                        	parte = 2
                        	Wait(time)
                            TriggerEvent("Notify","sucesso","Chamado recebido, entre na <b>Van</b>, e vá ao <b>local</b> entregar o <b>Medicamento</b>!")
                        	vRP.playSound("ATM_WINDOW","HUD_FRONTEND_DEFAULT_SOUNDSET")
                        	CriandoBlip(entregalocal,destino)
                        else
                        	TriggerEvent("Notify","importante","Voce colocou <b>"..quantidade.."/10 Medicamentos</b> na <b>Van</b>!")
		...
		if emservico and parte == 2 then
				...
				if model == 444171386 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    if IsControlJustPressed(0,58) and not andamento then
                    	if quantidade > 0 then
                            ...
                            TriggerEvent("Notify","importante","Voce retirou <b>1x Medicamento</b> da <b>Van</b>!")
                            vRP._CarregarObjeto("anim@heists@box_carry@","idle","apa_mp_h_acc_box_trinket_01",50,28422)
                            TirarCaixa()
                            andamento = true
                        else
                            parte = 1
                            vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                        	TriggerEvent("Notify","aviso","Voce entregou todos os seus <b>Medicamento</b>, volte para a Farmácia e pegue mais!")
                            quantidade = 0
                        end
					end
				end
			...
			if pegando and portaaberta2 and encomendapega and distance <= 1.5 and not IsPedInAnyVehicle(ped) then
                ...
                if model == 444171386 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    ...
                    if IsControlJustPressed(0,58) and andamento then
				        ...
				        ClearPedTasksImmediately(ped)
                        FreezeEntityPosition(ped,false)
                        TriggerEvent("Notify","importante","Voce colocou o <b>Medicamento</b> na <b>Van</b>, atualmente com <b>"..quantidade.."/10 Medicamentos</b>!")
					end
				end
			end
			if distance2 <= 50 and pegando and encomendapega then
				...
				if distance2 < 1.5 and not IsPedInAnyVehicle(ped) then
					DrawText3Ds(entregalocal[destino].x,entregalocal[destino].y,entregalocal[destino].z+0.35,"~b~[E] ~w~Entregar Medicamento")
					TriggerEvent('cancelando',false)
                    if IsControlJustPressed(0,38) then
                    	if quantidade >= 0 then
                    		Fade(1200)
                            ...
                            vRP._DeletarObjeto()
                            ClearPedTasksImmediately(ped)
                            TriggerServerEvent("vrp_medicamentos:receber",pagamento)
                            portaaberta = false
                            RemoveBlip(blip)
                        	if quantidade > 0 then
                        		TriggerEvent("Notify","sucesso","<b>Medicamento</b> entregue, voce ganhou <b>$"..pagamento.."</b>, faltam entregar <b>"..quantidade.."/10 <b>Medicamentos</b>!")
                                TriggerEvent("Notify","importante","Feche as portas traseiras, entre na <b>Van</b> e aguarde até receber uma <b>Entrega</b>!")
                                time = math.random(250,360)
                        	    Wait(time)
                        	    TriggerEvent("Notify","sucesso","Chamado recebido, entre na <b>Van</b>, e vá ao <b>local</b> entregar o <b>Medicamento</b>!")
                        	    vRP.playSound("ATM_WINDOW","HUD_FRONTEND_DEFAULT_SOUNDSET")
                                while true do
                                ...
                                portaaberta2 = false
                                portaaberta = false
                        	    TriggerEvent("Notify","aviso","Voce entregou todos os <b>Medicamentos</b>, volte para a Farmácia e pegue mais!")
                            end
                        end
					end
				end
			elseif distance2 <= 50 and not pegando and not encomendapega then
				...
				if distance2 < 1.5 and not IsPedInAnyVehicle(ped) then
					DrawMarker(21,entregalocal[destino].x,entregalocal[destino].y,entregalocal[destino].z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
					DrawText3Ds(entregalocal[destino].x,entregalocal[destino].y,entregalocal[destino].z+0.35,"~w~ Pegue o Medicamento na ~b~Van")
			    end
			end
		end
		Citizen.Wait(timeDistance)
		    ...
		    if model == 444171386 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                ...
                elseif traseira and not portaaberta then
	                ...
	                elseif parte == 2 and not encomendapega then
	            	    DrawText3Ds(coordsVan.x,coordsVan.y,coordsVan.z+0.80,"~b~[E] ~w~Fechar | ~b~[G] ~w~Pegar Medicamento")
	                elseif parte == 2 and encomendapega then
	            	    DrawText3Ds(coordsVan.x,coordsVan.y,coordsVan.z+0.80,"~b~[E] ~w~Fechar | ~b~[G] ~w~Guardar Medicamento")
	                end 
				    if IsControlJustPressed(0,38) then
...
function spawnVan()
	...
	while not HasModelLoaded(mhash) do
		...
		nveh = CreateVehicle(mhash,96.58, -215.88, 54.4+0.5,71.20,true,false)
...
function FadeRoupa(time,tipo,idle_copy)
	...
	if tipo == 1 then 
		...
		TriggerServerEvent("vrp_medicamentos:roupa")
	end
	DoScreenFadeIn(800)
end
...
function CaixaVan(veh,model)
	if quantidade <= 10 then
		RequestModel(GetHashKey("apa_mp_h_acc_box_trinket_01"))
        while not HasModelLoaded(GetHashKey("apa_mp_h_acc_box_trinket_01")) do
            Citizen.Wait(10)
        end
	    local coords = GetOffsetFromEntityInWorldCoords(veh,0.0,0.0,-5.0)
		if quantidade == 1 and model == 444171386 then
			propcaixa = nil
			propcaixa = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 2 and model == 444171386 then
			propcaixa2 = nil
			propcaixa2 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 3 and model == 444171386 then
			propcaixa3 = nil
			propcaixa3 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 4 and model == 444171386 then
			propcaixa4 = nil
			propcaixa4 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 5 and model == 444171386 then
			propcaixa5 = nil
			propcaixa5 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 6 and model == 444171386 then
			propcaixa6 = nil
			propcaixa6 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 7 and model == 444171386 then
			propcaixa7 = nil
			propcaixa7 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 8 and model == 444171386 then
			propcaix8 = nil
			propcaixa8 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 9 and model == 444171386 then
			propcaixa9 = nil
			propcaixa9 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
		...
		elseif quantidade == 10 and model == 444171386 then
			propcaix10 = nil
			propcaixa10 = CreateObject(GetHashKey("apa_mp_h_acc_box_trinket_01"),coords.x,coords.y,coords.z,true,true,true)
