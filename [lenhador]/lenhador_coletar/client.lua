...
Citizen.CreateThread(function()
	while true do
		...
		if not processo then
			for _,func in pairs(arvores) do
				...
				if distancia <= 50 and list[i] == nil then
					timeDistance = 5
					if distancia <= 10 and GetEntityModel(vehicle) == -667151410 then
						DrawMarker(21,x,y,z,0,0,0,0,180.0,130.0,0.5,0.5,0.5,250,100,50,150,1,0,0,1)
						if distancia <= 1.2  then
							drawTxt("PRESSIONE  ~r~E~w~  PARA ~y~CORTAR MADEIRA",4,0.5,0.90,0.50,255,255,255,200)
		...
		if processo then
			timeDistance = 5
			drawTxt("AGUARDE ~g~"..segundos.."~w~ SEGUNDOS ATÉ FINALIZAR A ~y~EXTRAÇÃO DA MADEIRA",4,0.5,0.90,0.50,255,255,255,200)
		end
