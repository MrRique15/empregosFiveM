...
Citizen.CreateThread(
	function()
		...
		while true do
			local timeDistance = 500
			if GetDistanceBetweenCoords(-344.98,-1562.29,25.24, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
				timeDistance = 5
				DrawMarker(21,-344.98,-1562.29,25.24 -0.4,0,0,0,0,0,0,1.5,1.5,1.0,255,230,100,150,0,0,0,1)
			end

			if GetDistanceBetweenCoords(-344.98,-1562.29,25.24, GetEntityCoords(GetPlayerPed(-1))) < 2.0 then
				timeDistance = 5
				if onjob == false then
					...
					if (IsControlJustReleased(1, 38)) then
						...
						TriggerServerEvent("colocar-uniforme:lixeiro")
						TriggerEvent("Notify", "aviso", "Pegue o caminhão e siga coletando o lixo pela cidade.")
						onjob = true
					end
				end
			end
...
Citizen.CreateThread(function()
	while true do
      ...
      if onjob then
		timeDistance = 5
		drawTxtOriginal("~y~PRESSIONE ~r~[F7] ~w~SE DESEJA FINALIZAR O SERVIÇO",4,0.270,0.95,0.35,255,255,255,200)
		if IsControlJustPressed(0,168) then
			onjob = false
			RemoveBlip(deliveryblip)
			SetWaypointOff()
			guardarCaminhao()
			TriggerServerEvent("tirar-uniforme:lixeiro")
...
function criaCaminhao()
	if not caminhao then
		...
		caminhao = CreateVehicle("trash",-341.89,-1559.17,25.24,170.0,true,false)
		NetworkFadeInEntity(caminhao,true)
		SetVehicleIsStolen(caminhao,false)
		SetVehicleNumberPlateText(caminhao,vRP.getRegistrationNumber())
		...
		SetModelAsNoLongerNeeded("trash")
	end
end

function guardarCaminhao()
	TriggerServerEvent("trydeleteveh",VehToNet(caminhao))
    caminhao = nil
end
...
function drawTxtOriginal(text,font,x,y,scale,r,g,b,a)
	SetTextFont(font)
	SetTextScale(scale,scale)
	SetTextColour(r,g,b,a)
	SetTextOutline()
