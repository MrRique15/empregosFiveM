...
local blips = nil
local selecionado = 0
local emservico = false --902.01,-172.76,74.08
local CoordenadaX = 902.01
local CoordenadaY = -172.76
...
local timers = 0
local payment = 10
local taxiCoords = {x = 911.98, y = -163.93, z = 74.38}
local carro = false
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		...
		if not emservico then
			...
			if distance <= 3 then
				timeDistance = 5
				DrawMarker(21,CoordenadaX,CoordenadaY,CoordenadaZ-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,0,0,0,50,0,0,0,1)
				if distance <= 1.2 then
					drawTxt("PRESSIONE  ~r~[E]~w~  PARA PEDIR UM EMPREGO",4,0.5,0.93,0.50,255,255,255,180)
					if IsControlJustPressed(0,38) then
						emP.addGroup()
						TriggerServerEvent("colocar-uniforme:taxista")
						emservico = true
						...
						TriggerEvent("Notify","bom","<b>Elysium:</b> Você está contratado(a)!",4000)
						TriggerEvent("Notify","bom","<b>Elysium:</b> Trate bem os passageiros!",5000)
					end
				end
			end
		end
	...
	while true do
		...
		if emservico then
			...
			if IsControlJustPressed(0,168) then
				...
				deleteCarro()
				emP.removeGroup()
				TriggerServerEvent("tirar-uniforme:taxista")
				TriggerEvent("Notify","aviso","<b>Downtown:</b> Agradecemos pelo serviço prestado!",5000)
			end
		end
		Citizen.Wait(timeDistance)
	end
...
function createCarro()
	if not carro then
		...
		carro = CreateVehicle("taxi",taxiCoords.x,taxiCoords.y,taxiCoords.z,198.33,true,false)
