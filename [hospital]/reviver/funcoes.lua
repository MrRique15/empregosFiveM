...
function GoToTargetWalking(target, vehicle, motorista)
    while emrota do
        ...
        if distanceToTarget <= 2 and not norunrange then
            ...
            startAnim(motorista,"missminuteman_1ig_2", "tasered_1")-- Usa a função criada por você para iniciar uma animação
            Citizen.Wait(5000)
...
function IrEmbora(vehicle, motorista)
    TaskVehicleDriveWander(motorista, vehicle, 17.0, drivingStyle) --FiveM/ faz o npc ir ao carro e dirigir.
    Citizen.Wait(100)
    SetEntityAsNoLongerNeeded(vehicle)--FiveM/ Dispensa a entidade
    SetPedAsNoLongerNeeded(motorista)--FiveM/ Dispensa o Ped
    RemoveBlip(samuBlip)
TriggerServerEvent("reviver:Player")
    Citizen.Wait(10000)
