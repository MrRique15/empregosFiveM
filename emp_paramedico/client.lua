...
local CoordenadaX = 310.21      
local CoordenadaY = -599.61     -- 310.21, -599.61, 43.3
			...
			if distance <= 3 then
				...
				if distance <= 1.2 then
					...
					if IsControlJustPressed(0,38) and emP.checkPermission() then
						...
						TriggerServerEvent("colocar-uniforme:medico")
					end
				end
			end
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if emservico then
			...
			if distance <= 50.0 and IsVehicleModel(vehicle,GetHashKey("paramedicoambu")) then
	...
	while true do
		...
		if emservico then
			...
			if IsControlJustPressed(0,168) then
				...
				checkped = true
				emservico = false
				TriggerServerEvent("tirar-uniforme:medico")
