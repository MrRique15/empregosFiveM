					...
					if IsControlJustPressed(1,38) and lastVehicle == 1026149675 and emp.checkPlate(lastVehicle) and emp.checkCrimeRecord() then
						if not working then
							...
							if parseInt(hour) >= 06 and parseInt(hour) <= 20 then
								...
								TriggerServerEvent("colocar-uniforme:leite")
							else
...
Citizen.CreateThread(function()
	while true do
		...
		if working then
			timeDistance = 5
			drawTexts("~y~PRESSIONE ~r~[F7] ~w~SE DESEJA FINALIZAR O SERVIÇO",4,0.270,0.95,0.35,255,255,255,200)
			if IsControlJustPressed(0, 168) then
				working = false
				RemoveBlip(blips)
				TriggerServerEvent("tirar-uniforme:leite")
			end
		end
