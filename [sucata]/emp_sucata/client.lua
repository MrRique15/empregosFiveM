...
Citizen.CreateThread(function()
	...
	while true do
		local timeDistance = 500
		if GetDistanceBetweenCoords(-460.39862060547,-1715.9298095703,18.640439987183, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
			timeDistance = 5
			DrawMarker(23,-460.39862060547,-1715.9298095703,18.640439987183 - 0.9001,0,0,0,0,0,0,10.0,10.0,1.0,0,95,140,50,0,0,0,0)
		end
		if GetDistanceBetweenCoords(-460.39862060547,-1715.9298095703,18.640439987183, GetEntityCoords(GetPlayerPed(-1))) < 2.0 then
			timeDistance = 5
			if onjob == false then
				...
				if (IsControlJustReleased(1, 38)) then
					if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("scrap")) then
						...
						onjob = true
						TriggerServerEvent("colocar-uniforme:sucata")
					else
						Citizen.Wait(1)
	...
	while true do
		if onjob then
			...
			if IsControlJustPressed(0, 168) then
				...
				SetWaypointOff()
				TriggerServerEvent("tirar-uniforme:sucata")
